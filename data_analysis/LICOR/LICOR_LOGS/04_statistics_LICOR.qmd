...
### Normality of *ALL* Observations  
Plots of the original data are shown next to the wrangled data.  
Wrangler graph data has outliers, as well as invalid data removed.  
```{r graph_means}
#| echo: false
#| warning: false
...
Graphs are to visually check if the daily averages are normal.
...
### Graph Data
qqnorm() is used to generate Q-Q plots for all data.  
The Treatment data is not normal. A transformation of log() is applied to make it normal.  
Control and Reference are already normal.  
```{r QQ_plots}
#| echo: false
#| warning: false
...
qqnorm(TreatContRef[TreatContRef$Data_leaftype == "Treatment",]$gm,
       main = paste("Original: Q-Q Plot Elevation ", ELEVATION, ": \nTreatment - Daily Mean GaseEx_A", sep = ''))
qqline(TreatContRef[TreatContRef$Data_leaftype == "Treatment",]$gm)
# shapiro.test(Treat)

# add in a log() column to TreatContRef
gmlog <- log(TreatContRef$gm)
TreatContRef$gmLog <- log(TreatContRef$gm)
...
# Control data normal already, don't need this
# qqnorm(TreatContRef[TreatContRef$Data_leaftype == "Control",]$gmLog,
#        main = paste("Q-Q Plot Elevation ", ELEVATION, "\nControl - Daily Mean GaseEx_A", sep = ''))
# qqline(TreatContRef[TreatContRef$Data_leaftype == "Control",]$gmLog)
...
# Reference data normal already, don't need this
# qqnorm(TreatContRef[TreatContRef$Data_leaftype == "Reference",]$gmLog,
#        main = paste("Q-Q Plot Elevation ", ELEVATION, "\nReference - Daily Mean GaseEx_A", sep = ''))
# qqline(TreatContRef[TreatContRef$Data_leaftype == "Reference",]$gmLog)
# shapiro.test(Ref)
...
Apply a log() transformation and the new graph looks normal.  
  
The Control and Reference Q-Q plots look normal.

## Test Normality using Statistics

**Shapiro-Wilk Test**  
If the p-value is less than $α$ =.05, the sample does not come from a normally distributed population.  
H0: The data is normally distribued    
HA: The data is not normally distribued  

### Treatment vs Transformation  
The orignal Treatment data is not normal. There is a drastic improvement in the p-val when the log() transformation is applied, which then shows the data is normal.  
**Treatment**  
The p-value = 0.03515 for the Treatment data is significant at $α$ =.05.  
The original Treatment data is not normal.  

**Treatment log() Transformation**  
The p-value = 0.6415 for the log() transformed data is not significant at $α$ =.05.  
The transformed Treatment data is normal.  

### Control and Reference  
**Control**\
The p-value = 0.7283 for the Control data is not significant at $α$ =.05.  
The Control data is normal.  

**Reference**\
The p-value = 0.1215 for Reference data is not significant at $α$ =.05.  
the Reference data is normal.  

```{r}
#| echo: false
#| warning: false

shapiro.test(TreatContRef[TreatContRef$Data_leaftype == "Treatment",]$gm)
shapiro.test(TreatContRef[TreatContRef$Data_leaftype == "Treatment",]$gmLog)

shapiro.test(TreatContRef[TreatContRef$Data_leaftype == "Control",]$gm)
# shapiro.test(TreatContRef[TreatContRef$Data_leaftype == "Control",]$gmLog)

shapiro.test(TreatContRef[TreatContRef$Data_leaftype == "Reference",]$gm)
# shapiro.test(TreatContRef[TreatContRef$Data_leaftype == "Reference",]$gmLog)
...
all2 <-  ggplot() +
  ...
  # scale_y_continuous(limits=c(0.0, 0.1)) +#, breaks=c(0, 2.5, 3.0, 3.5)) +
  # ggtitle(paste("Elevation ", ELEVATION, " - Daily Mean Observations on GasEx_A")) +
  ggtitle(paste("log() Tranformed Treatment, Original Control data\nElevation ", ELEVATION, " Daily Mean GaseEx_A", sep = '')) +
  xlab("Leaf Type") +
  ylab("GasEx_A")
...
The p-value = 0.2291 which is greater than alpha = .05, we fail to reject H0.  
The variance among variables are equal.  
...
bartlett.test(gm ~ Group, data = finalblt)


```

## Welch's t-test  
Used to compare the means between two independent groups with unequal variance.  
H0: difference in means is 0  

## Two Sample T-Test  
Test whether or not the means of two populations are equal.  

Normality assumption was achieved with the log() transformation of Treatment.  
*A Two Sample T-Test can be used since the variance is also equal.*  

The p-value = 9.141e-08 which is less than alpha = .05, t = -8.1264 does not fall in the 95% confidence interval [-3.564647, -2.108434]. We reject the null hypothesis.  
The means are different.  

```{r}
# Two Sample T-Test 
# variance is equal
t.test(TreatContRef[TreatContRef$Data_leaftype == "Treatment",]$gmLog, 
       TreatContRef[TreatContRef$Data_leaftype == "Control",]$gm,
       var.equal=TRUE)
...
Normality could be achieved for Treatment when a transformation of log() was applied. Control and Reference data was already normal. The variances were shown to be equal.  
__A Two Sample t-test was performed and it showed that the means between the Treatment and Control are not equal.__
