...
# library(MASS)
# b <- boxcox(lm(TreatContRef[TreatContRef$Data_leaftype == "Treatment",]$gm ~ 1))
# # Exact lambda
...
# # gmlog <- log(TreatContRef$gm) # WRONG!!!!
...
```
Normality with Unequal Variance

Normality with Equal Variance
...
**A Two Sample t-test was performed and it showed that the means between the Treatment and Control are not equal.**
