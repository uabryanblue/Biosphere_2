...
scalefactor <- 0.34
tscale <- max(mean(tdata[[VARNAME]])) * scalefactor
cscale <- max(mean(cdata[[VARNAME]])) * scalefactor
rscale <- max(mean(rdata[[VARNAME]])) * scalefactor

# keep the scale the same for all plots, find the max of all datasets
finalscale <- max(tscale, cscale, rscale)
finalscale
