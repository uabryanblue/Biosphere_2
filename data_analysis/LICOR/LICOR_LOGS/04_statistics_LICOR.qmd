...
mean_plant_data <- function(plot_data, plantid, sysobs) {
  ...
  plotdf <- plotdf %>%
    ...
    # variables are too variable use median instead of mean
  ...
  nplot <- ggplot(plotdata[plotdata$Data_leaftype==leaftype,], aes(x = get(dfvariable))) +
    stat_function(
      ...
      args = with(plotdata, 
                ...
                c(mean = median(cdata[[dfvariable]], na.rm = TRUE), 
                # c(mean = mean(cdata[[VARNAME]], na.rm = TRUE), 
                    sd = sd(get(dfvariable))))
    ) +
...
finaldata <- read_and_clean_data(filename)
...
Q1 <- quantile(finalTreat[[VARNAME]], .25, na.rm = TRUE)
Q3 <- quantile(finalTreat[[VARNAME]], .75, na.rm = TRUE)
IQR <- IQR(finalTreat[[VARNAME]], na.rm = TRUE)
...
Q1 <- quantile(finalCont[[VARNAME]], .25, na.rm = TRUE)
Q3 <- quantile(finalCont[[VARNAME]], .75, na.rm = TRUE)
IQR <- IQR(finalCont[[VARNAME]], na.rm = TRUE)
# subset data where points value is outside 1.5*IQR of Q1 and Q3
...
Q1 <- quantile(finalRef[[VARNAME]], .25, na.rm = TRUE)
Q3 <- quantile(finalRef[[VARNAME]], .75, na.rm = TRUE)
IQR <- IQR(finalRef[[VARNAME]], na.rm = TRUE)
# subset data where points value is outside 1.5*IQR of Q1 and Q3
...
tdata <- finaldata %>% filter(Data_leaftype == "Treatment") %>% arrange(SysObs_date)
cdata <- finaldata %>% filter(Data_leaftype == "Control") %>% arrange(SysObs_date)
rdata <- finaldata %>% filter(Data_leaftype == "Reference") %>% arrange(SysObs_date)

# compute a reasonable scale for the Y-axis
# scalefactor = calculate this percent above the maximum mean for each dataset
# TODO: this is wrong since the graph is dnorm()
scalefactor <- 0.34
tscale <- max(mean(tdata[[VARNAME]], na.rm = TRUE)) * scalefactor
cscale <- max(mean(cdata[[VARNAME]], na.rm = TRUE)) * scalefactor
rscale <- max(mean(rdata[[VARNAME]], na.rm = TRUE)) * scalefactor

# keep the scale the same for all plots, find the max of all datasets
finalscale <- max(tscale, cscale, rscale, na.rm = TRUE)
finalscale

# TREATMENT
# dnorm plot for original and wrangled  data
t1 <- ggplot(tdata, aes(x = tdata[[VARNAME]])) +
  stat_function(
    fun = dnorm,
    args = with(tdata, 
                # data is too variable, use median
                c(mean = median(tdata[[VARNAME]], na.rm = TRUE), 
                # c(mean = mean(tdata[[VARNAME]], na.rm = TRUE), 
                  sd = sd(tdata[[VARNAME]], na.rm = TRUE)))
  ) +
  ggtitle("dnorm Graphs") + # for the main title
  ...
  scale_y_continuous(limits = c(0.0, finalscale)) # TODO change the scale of data values?
...
t2 <- ggplot(newTreat, aes(x = newTreat[[VARNAME]])) +
  stat_function(
    ...
    args = with(newTreat, 
                # data is too variable, use median
                c(mean = median(newTreat[[VARNAME]], na.rm = TRUE),
                  # c(mean = mean(newTreat[[VARNAME]], na.rm = TRUE), 
                  sd = sd(newTreat[[VARNAME]], na.rm = TRUE)))
  ) +
  scale_x_continuous(paste("Treatment - Wrangled values of ", VARNAME, sep = '')) +
  scale_y_continuous(limits = c(0.0, finalscale))
...
c1 <- ggplot(cdata, aes(x = cdata[[VARNAME]])) +
  stat_function(
    ...
    args = with(cdata, 
                # data is too variable for mean use median
                c(mean = median(cdata[[VARNAME]], na.rm = TRUE), 
                # c(mean = mean(cdata[[VARNAME]], na.rm = TRUE), 
                  sd = sd(cdata[[VARNAME]], na.rm = TRUE)))
  ) +
...
c2 <- ggplot(newCont, aes(x = newCont[[VARNAME]])) +
  stat_function(
    ...
    args = with(newCont, 
                # data is too variable for mean use median
                c(mean = median(newCont[[VARNAME]], na.rm = TRUE), 
                # c(mean = mean(newCont[[VARNAME]], na.rm = TRUE), 
                  sd = sd(newCont[[VARNAME]], na.rm = TRUE)))
  ) +
...
r1 <- ggplot(rdata, aes(x = rdata[[VARNAME]])) +
  stat_function(
    ...
    args = with(rdata, 
                  # data is too variable for mean use median
                c(mean = median(rdata[[VARNAME]], na.rm = TRUE), 
                # c(mean = mean(cdata[[VARNAME]], na.rm = TRUE),  
                  sd = sd(rdata[[VARNAME]], na.rm = TRUE)))
  ) +
...
r2 <- ggplot(newref, aes(x = newref[[VARNAME]])) +
  stat_function(
    ...
    args = with(newref, 
                  # data is too variable for mean use median
                c(mean = median(newref[[VARNAME]], na.rm = TRUE), 
                # c(mean = mean(cdata[[VARNAME]], na.rm = TRUE), 
                  sd = sd(newref[[VARNAME]], na.rm = TRUE)))
  ) +
  scale_x_continuous(paste("Reference - Wrangled values of ", VARNAME, sep = '')) +
  scale_y_continuous(limits = c(0.0, finalscale))

# plots of same data sets side by side, in 3 rows of 2 columns
  (t1 + t2) /
  (c1 + c2) /
  (r1 + r2)
# ------------------------------------------------
...
all1 <-  ggplot() +
  ...
  ggtitle(paste("Elevation ", ELEVATION, "\nDaily Median Observations on ", VARNAME, sep="")) +
  xlab("Leaf Type") +
  ylab(VARNAME)
