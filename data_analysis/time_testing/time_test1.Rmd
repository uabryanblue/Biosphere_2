...
test <- read_csv(
  file = "~/Biosphere_2/raw_data/Time_Testing/8CAAB54D7F18_SYSLOG_C45BBEE4FD71.log",
  ...
  col_types = cols(`2` = col_integer(),
                   `32` = col_integer(),
                   `2023-08-18 13:32:01...1` = col_datetime(format = "%Y-%m-%d %H:%M:%S"),
                   `2023-08-18 13:32:01...2` = col_datetime(format = "%Y-%m-%d %H:%M:%S")),
            trim_ws = FALSE)
...
ggplot(data = test, aes(x = log_date)) +
         ...
         ggtitle("8/19/2023 Test Time Drift on Remote ESP8266 vs one with RTC module") +
         xlab("Time (2 minute intervals)") + 
